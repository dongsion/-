...
function openDetail(id) { const item = state.sleeves.find((entry) => entry.id === id); if (!item) return; $("#detailContent").innerHTML = `<div class="detail-art" style="background:linear-gradient(135deg, ${item.colors[0]}25, #eeefef)">${visualHtml(item)}</div><div class="detail-body"><span class="tag ${item.tag === "常规款" ? "normal" : ""}">${escapeHtml(item.tag)}</span><h2>${escapeHtml(item.name)}</h2><span class="detail-brand">${escapeHtml(item.brand)} · ${escapeHtml(item.enBrand || "")}</span><p>${escapeHtml(item.story || "这枚杯套还没有故事，等你来补充。")}</p><div class="detail-date">发行时间 · ${escapeHtml(item.date)}</div></div>`; $("#detailModal").hidden = false; }
